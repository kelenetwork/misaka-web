...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.update(applications).set({ status: "approved", reviewedBy: guard.user.id, reviewedAt: new Date() }).where(eq(applications.id, id));
  await logAudit(guard.user.id, "application.approved", id, { createdUserId: user.id, tempPassword, emailTodo: true }, getIp(req));
